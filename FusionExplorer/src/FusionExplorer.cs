    ...
    public partial class FusionExplorer : Form 
    {
        ...
        private void import_file(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                new_new_import_file(new_file_data);
            }
        }
        ...
        {
            ArchiveFile selected_archive_file = get_selected_archive_file();
            // new file is smaller or same size
            // doesn't require inserting new data and updating data offsets
            // can just overwrite previous data and update file entry
            if (data.Length <= selected_archive_file.file_entry.Decompressed_Size)
            {
                ...
                if (selected_archive_file.file_entry.Compression_Flag == 1 || selected_archive_file.file_entry.Compression_Flag == 8)
                {
                    ...
                    using (MemoryStream ms = new MemoryStream(pak_data))
                    {
                        ...
                        selected_archive_file.file_entry.Compressed_Size = compressed.Length;
                        selected_archive_file.file_entry.Decompressed_Size = data.Length;

                        // write new data
                        ms.Position = selected_archive_file.file_entry.Data_Offset;
            ...
            {
                /*
                file is bigger than previous
                this requires updating all the data offsets for the file entries after the file being imported due to the increase data size for this file
                also requires splitting the file into two pieces and piecing it back together with the new data in the middle

                [a] <- beginning stride (header, file entries, file data of files before one being replaced)
                [b] <- file data we are replacing
                [c] <- ending stride (file data of files after one being replaced, filenames, footer)

                simply:
                    1.
                    recalculate data offsets with new sizes of the new file

                    2.
                    we copy [a] into buffer
                    we copy [c] into buffer

                    3.
                    we reconstruct pak data with new file data [b] in the middle
                */
                ...
                if (selected_archive_file.file_entry.Compression_Flag == 1 || selected_archive_file.file_entry.Compression_Flag == 8)
                {
                    ...
                    using (MemoryStream ms = new MemoryStream(pak_data))
                    {
                        ...
                        // [b] - new file data
                        // [c] - ending stride
                        byte[] new_pak_data = new byte[beginning.Length + compressed.Length + ending.Length];
                        Buffer.BlockCopy(beginning, 0, new_pak_data, 0, beginning.Length);
                        Buffer.BlockCopy(compressed, 0, new_pak_data, beginning.Length, compressed.Length);
                        Buffer.BlockCopy(ending, 0, new_pak_data, beginning.Length + compressed.Length, ending.Length);
                ...
                else
                {
                    using (MemoryStream ms = new MemoryStream(pak_data))
                    {
                        ...
                        // [b] - new file data
                        // [c] - ending stride
                        byte[] new_pak_data = new byte[beginning.Length + data.Length + ending.Length];
                        Buffer.BlockCopy(beginning, 0, new_pak_data, 0, beginning.Length);
                        Buffer.BlockCopy(data, 0, new_pak_data, beginning.Length, data.Length);
                        Buffer.BlockCopy(ending, 0, new_pak_data, beginning.Length + data.Length, ending.Length);
        ...
        private void new_import_file(byte[] data)
        {
            ArchiveFile selected_archive_file = get_selected_archive_file();
        ...
        private void new_new_import_file(byte[] data)
        {
            ArchiveFile selected_archive_file = get_selected_archive_file();
